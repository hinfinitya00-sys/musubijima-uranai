...
## 2026-09-16 無料範囲・会員限定表示の最終監査

- 導カード、結び族、歌みくじは、未契約ユーザーでも結果全文を無料表示する仕様へ統一した。
- み・たまカード、今年の運勢、ネガティブ神は冒頭2行を表示し、続きだけをぼかして「この続きは会員限定です。」と月額330円導線を表示する仕様へ統一した。
- 会員プラン画面の対象説明を上記仕様と一致させ、無料対象を明記した。占い本文、画像、ピンク／白のブランド配色は変更していない。
- 結果画面のPC最大幅を760px（み・たまカード内は720px）へ調整し、1440px表示の過剰な左右余白を削減した。320px、390px、768px、1440px、PC 200%相当の全条件で横スクロール、操作要素の欠け、壊れた画像を検査した。
- 全6占い画面の結果本文は、Webで `user-select: none`、iOS Safariで長押し選択抑止、ネイティブでText選択不可となる既存コピー防止を維持した。操作フロー試験で各画面の対象本文にコピー防止スタイルが継承されることを検証した。
- `e2e/release-flows.spec.ts` は5表示条件×8フローの40/40件に合格。無料全文、会員限定ぼかし、案内文、330円プラン遷移、余白・収まり、コピー防止を検証した。
- GitHub Pages反映後も同じ40フローを公開URLへ実行し40/40件に合格。本番CDNの画像ロード完了後に自然サイズを検査し、画像欠損0件を確認した。
- `e2e/release-visual-audit.spec.ts --grep '/fortune/'` は6画面×5表示条件の30/30件に合格。直前の公開24ルート全体監査120/120件と合わせ、変更対象とサイト全体の回帰を確認した。
- 証跡画像は `artifacts/release-screenshots/` に表示条件別で保存した。

## 2026-09-17 ホーム画像比率の再監査

- ホームの導カード（1200×284）とみ・たまカード（1434×436）を116×116の共通正方形枠へ入れていたため、横長画像の実表示が極端に小さくなる不備を修正した。
- 両画像をカード内幅いっぱい（カード幅の80%以上）で表示し、元画像の縦横比を明示的なレスポンシブ高さへ反映。み・たまカードは画像・見出し・説明・CTAの上下構成へ整理し、画像枠内の過剰な上下余白を解消した。
- 文言、画像データ、ピンク／白の配色は変更していない。画像へアクセシビリティラベルを追加した。
- 320px、390px、768px、1440px、PC 200%相当で画像占有率と縦横比を自動検査し5/5件合格。全操作フロー45/45件、公開24ルート×5条件の表示監査120/120件に合格した。
- PC・モバイルのカード単位の目視証跡を `artifacts/release-screenshots/*-home-*-prominent.png` に保存した。

## 2026-09-20 月額決済手段の拡張・監査

- 月額330円のStripe Checkoutで決済手段をコード固定していた指定を撤去し、Stripe Dashboardの設定と購入者の端末・地域・通貨・継続課金対応状況に基づく動的決済手段へ変更した。
- 対応案内はカード／デビットカード、Apple Pay、Google Pay、Linkへ統一した。PayPayはStripeの継続課金に対応しないため、月額会員の決済手段には含めない。
- 会員プラン画面と特定商取引法に基づく表記を更新し、対応端末に利用可能な手段だけがStripe Checkoutへ表示されることを明記した。
- 回帰防止テストに、Checkoutが `payment_method_types` を固定しないこと、subscriptionモードを維持すること、決済案内と法的表記が一致することを追加した。
...
## 2026-09-21 認証メール導線の実地監査

- 利用者が登録後に開いた404は、むすび島の認証メールではなく、別リポジトリ `hia-tester` のGitHub Actions失敗通知だった。対象実行は定期実行されたテストの `Upload Report` 失敗で、むすび島のユーザー登録処理とは独立している。
- Supabase本番環境でMagic Linkを生成し、認証リンクのホストが `aymjfldowdyxlypdnxok.supabase.co`、戻り先が `https://hinfinitya00-sys.github.io/musubijima-uranai/oauth/callback` であることを秘密トークンを出力せず確認した。公開コールバックへの直接GETはHTTP 200、末尾スラッシュ付きURLはGitHub Pagesの仕様上404となるため、アプリ・Supabaseとも末尾スラッシュなしへ固定した。
- `supabase/config.toml` に本番Site URLと許可コールバックURLをコード管理し、設定の欠落を単体テストで検知するようにした。
- ログイン・新規登録の送信完了画面へ、むすび島認証メールとGitHub等の別通知を区別する案内、迷惑メール／プロモーション確認、同一アドレスへの再送導線を追加した。
- 単体テスト39/39、型検査、本番ビルド36ルート、認証3画面×5表示条件の視覚監査15/15、送信完了・再送導線5/5に合格した。全主要フローは5表示条件の50件を実行し、停止していた旧ローカル配信プロセスを再起動後、全ケースの合格を確認した。
